...
def get_webcam_frames(batch_size):
...
class YolopTRT(object):
    ...
    def draw_labels(self, img, result_boxes, result_scores, result_classid, segout, laneout):
        for i in range(len(result_boxes)):
            ...
            box_pixel = [int(coord * img.shape[1]) for coord in box]

            # Draw bounding box and label on the image
            self.plot_one_box(box_pixel, img, color=color, label=f"{label} {score:.2f}")
    ...
    def infer(self, raw_image_generator):
        ...
        # Process each image in the generator
        for i, image_raw in enumerate(raw_image_generator):
            if i >= self.batch_size:
                break  # Avoid exceeding the batch size
        ...
        # Process the output for each image
        for i in range(min(self.batch_size, len(batch_image_raw))):
            ...
            self.draw_labels(img, result_boxes, result_scores, result_classid, segout[i], laneout[i])
    ...
    def preprocess_image(self, raw_image):
        ...
        scale_w = self.input_w / w
        scale_h = self.input_h / h
        ...
        # Create a blank canvas with the desired input size and place the resized image in the center
        canvas = np.zeros((self.input_h, self.input_w, 3), dtype=np.uint8)
        x_offset = (self.input_w - new_w) // 2
        y_offset = (self.input_h - new_h) // 2
        canvas[y_offset:y_offset + new_h, x_offset:x_offset + new_w] = image

        # Normalize and preprocess the image
        canvas = canvas.astype(np.float32)
        canvas /= 255.0
        canvas = (canvas - (0.485, 0.456, 0.406)) / (0.229, 0.224, 0.225)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for batch in get_webcam_frames(yolop_wrapper.batch_size):
